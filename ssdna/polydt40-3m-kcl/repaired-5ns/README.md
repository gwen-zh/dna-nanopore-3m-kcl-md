# Repaired poly(dT)40 production in 3 M KCl
...
Stage endpoint checks are in [`validation/`](validation/), and the full
production geometry trace is
[`validation/prod5.metrics.csv`](validation/prod5.metrics.csv). The exact code
snapshot used by the job is in [`code/`](code/). `dna.pdb`, `dna.psf` and
`donor.json` document the reconstructed DNA donor.

[`validation/production_all_frame_summary.json`](validation/production_all_frame_summary.json)
...
[`data-manifest/repaired_dt_5ns.sha256`](../../../data-manifest/repaired_dt_5ns.sha256).
